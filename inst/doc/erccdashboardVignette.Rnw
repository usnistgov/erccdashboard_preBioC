...
\subsection{Initialize the expDat List}
\verb|initDat|


\subsection{Estimate the mRNA Fraction Difference Between Samples}
\verb|est_r_m|

\subsection{Assess Dynamic Range}
...
\subsection{Assess Diagnostic Performance with ROC Curve and AUC Statistic}
expDat <- erccROC(expDat)


\subsection{Estimate Limit of Detection of Ratios}

expDat = estLODR(expDat, kind = "ERCC", prob = 0.9)
expDat = estLODR(expDat, kind = "Sim", prob = 0.9)


\subsection{Apply LODR to MA plots and Gene Lists}

expDat <- annotLODR(expDat)


\subsection{Saving Figures to PDF}
saveERCCPlots(expDat)


\subsection{Saving expDat to an RData file}
    cat("\nSaving expDat list to .RData file...")
    nam <- paste(expDat$sampleInfo$filenameRoot, "expDat", sep = ".")
    assign(nam, expDat)
    to.save <- ls()
    save(list = to.save[grepl(pattern = nam, x = to.save)], file = paste0(expDat$sampleInfo$filenameRoot, 
        ".RData"))
    cat("\nAnalysis completed.")
