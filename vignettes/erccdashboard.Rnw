...
Experiments for the SEQC interlaboratory and interplatform analysis were 
performed with aliquots from single large batches of commercially available 
reference RNA samples, Universal Human Reference RNA (UHRR) and Human Brain 
Reference RNA (HBRR). The count data and microarray data provided here are 
from experiments using the same original UHRR and HBRR samples.

\subsection{Analysis of an UHRR vs. HBRR RNA-Seq experiment}
The following command can be used to analyze the example RNA-Seq:
<<SEQCMainCount,eval=FALSE, keep.source=TRUE>>=
exDat <- runDashboard(datType="count", isNorm = FALSE,
...
following command:
<<SEQCMainArray,eval=FALSE, keep.source=TRUE>>=
