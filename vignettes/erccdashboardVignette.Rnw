...
estimation.
In the case of count data if QuasiSeq DE testing has already been done and 
a correctly formatted csv file is provided with the
...
To use results from another DE testing tool (instead of QuasiSeq) a csv file with
the name "filenameRoot ERCC pvals.csv" and correct column headers should be in the working directory.
For RNA-Seq ("count") data, this file is required to complete analysis without 
QuasiSeq DE testing or if the input data (exTable) is already normalized 
(isNorm = TRUE).
