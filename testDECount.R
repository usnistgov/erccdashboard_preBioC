...
#                       rep(use.res$d0[2],nrow(simcnt)))
  ...
  quasiDispPlot = ggplot() + geom_point(data = dispcnt, aes(x = mean, y = y),
                                        ...
                                        alpha = 0.6) + geom_point(data = dispERCC, aes(x = mean, y = y,colour = Ratio), size = 5, alpha = 0.6) + xlab("Mean Counts") + ylab("log(Quasi Dispersion)") + stat_smooth(data = dispcntSort,aes(x = xsort, y = ysort),colour = "black") + scale_x_log10() + colScale + theme(legend.justification=c(1,1), legend.position=c(1,1)) + theme_bw()
